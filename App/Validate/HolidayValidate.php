...
class HolidayValidate extends AbstractDataRepositoryValidation
{
    ...
    public function validateBeforePersist(Collection $entityCollection, ?object $dataRepository = null, ?object $controller = null): array
    {
        ...
        $catName = (isset($dataCollection['slug']) && $dataCollection['slug'] !=='' ? $dataCollection['slug'] : $dataCollection['name']);

        $newCleanData = [];
        if (null !== $dataCollection) {
            $newCleanData = [
                'name' => $this->isSet('name', $dataCollection, $dataRepository),
                'slug' => Utilities::titleSlugConverter($catName),
    ...
    private function throwWarningIfNoChange(Collection $entityCollection, ?object $dataRepository = null)
    {
        if ($dataRepository !== null) {
            if (
                $entityCollection['name'] === $dataRepository->cat_name &&
                $entityCollection['slug'] === $dataRepository->cat_slug
            ) {
