...
use App\Controller\Admin\EmployeeController;
...
class EmployeeValidate extends AbstractDataRepositoryValidation
{
    ...
    /** @var string */
    protected const REDIRECT_BACK_TO = '/admin/employee/index';
    ...
    {
        $this->rules = $rules;
        $this->rules->addObject(EmployeeController::class, $this);
    }
    ...
    private function throwWarningIfNoChange(Collection $entityCollection, ?object $dataRepository = null)
    {
        if ($dataRepository !== null) {
            if (
                $entityCollection['cat_name'] === $dataRepository->cat_name &&
    ...
    public function validate(Collection $entityCollection, ?object $dataRepository = null): void
    {
        $this->doValidation(
            ...
            function ($key, $value, $entityCollection, $dataRepository) {
                if ($rules = $this->rules) {
                    return match ($key) {
                        'cat_name' => $rules->addRule("required"),
