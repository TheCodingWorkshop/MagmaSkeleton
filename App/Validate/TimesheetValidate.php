...
class TimesheetValidate extends AbstractDataRepositoryValidation
{
    ...
    /** @var string */
    protected const REDIRECT_BACK_TO = '/admin/timesheet/index';
    ...
    {
        $this->rules = $rules;
        $this->rules->addObject(TimesheetController::class, $this);
    }
    ...
    public function validateBeforePersist(Collection $entityCollection, ?object $dataRepository = null, ?object $controller = null): array
    {
        ...
        $TimesheetName = (isset($dataCollection['slug']) && $dataCollection['slug'] !=='' ? $dataCollection['slug'] : $dataCollection['name']);
        $newCleanData = [];
        if (null !== $dataCollection) {
            $newCleanData = [
                'name' => $this->isSet('name', $dataCollection, $dataRepository),
                'slug' => Utilities::titleSlugConverter($TimesheetName),
                'description' => $this->isSet('description', $dataCollection, $dataRepository),
                'Timesheet_date' => !$dataRepository ? $controller->repository->getPublicBankTimesheets($TimesheetName) : 'None',
