...
class ProjectValidate extends AbstractDataRepositoryValidation
{
    ...
    /** @var string */
    protected const REDIRECT_BACK_TO = '/admin/project/index';
    ...
    {
        $this->rules = $rules;
        $this->rules->addObject(ProjectController::class, $this);
    }
    ...
    public function validateBeforePersist(Collection $entityCollection, ?object $dataRepository = null): array
    {
        ...
        if (null !== $dataCollection) {
            $newCleanData = [
                ...
                'client' => $this->isSet('client', $dataCollection, $dataRepository),
                'coordinator' => $this->isSet('coordinator', $dataCollection, $dataRepository),
                'type' => $this->isSet('client', $dataCollection, $dataRepository),
                'description' => $this->isSet('description', $dataCollection, $dataRepository),
                'duration' => $this->isSet('duration', $dataCollection, $dataRepository),
                'location' => $this->isSet('location', $dataCollection, $dataRepository),
                'cost' => $this->isSet('cost', $dataCollection, $dataRepository),
    ...
    private function throwWarningIfNoChange(Collection $entityCollection, ?object $dataRepository = null)
    {
        if ($dataRepository !== null) {
            if (
                ...
                $entityCollection['client'] === $dataRepository->client
            ) {
    ...
    public function validate(Collection $entityCollection, ?object $dataRepository = null): void
    {
        $this->doValidation(
            ...
            function ($key, $value, $entityCollection, $dataRepository) {
                if ($rules = $this->rules) {
                    return match ($key) {
                        'name', 'client', 'location', 'duration', 'coordinator', 'cost', 'description' => $rules->addRule("required"),
